/*	Author: Vicor A. Colton
	...
	Run:	 mpirun -np <number of processes> ./Boids 
			[boids] [loops] [k] [maxv] [acc] [width] [height] [SF1] [SF2] [MIN] [SF3] [SF4]
...
#define BOIDS	50		// Number of boids to run
#define WIDTH	1000		// Width of the program area
#define HEIGHT	1000		// Height of the program area
#define MAXV	10		// Maximum boid velocity
#define ACC		1.25	 	// Boid acceleration
#define K		7		// K nearest neighbors

// -------------------< Default rule handling parameters >-------------------
#define SF1		1.0		// Scale factor 1 - attraction
#define SF2		32.0	// Scale factor 2 - repulsion
#define MIN		50.0	// The minimum distance between boids before repulsion 
#define SF3		8.0		// Scale factor 3 - alignment
#define SF4		10.0	// Scale factor 4 - bounding - unused
...
void parseargs(int argc, char **argv, int *boids, int *loops, int *k, 
			   ...
			   double *sf1, double *sf2, double *min, double *sf3, double *sf4)
{
	...
	if(argc > 8)// Parse height
		*sf1 = atof(argv[8]);
	else
		return;

	if(argc > 9)// Parse height
		*sf2 = atof(argv[9]);
	else
		return;
		
	if(argc > 10)// Parse height
		*min = atoi(argv[10]);
	else
		return;
		
	if(argc > 11)// Parse height
		*sf3 = atoi(argv[11]);
	else
		return;

	if(argc > 12)// Parse height
		*sf4 = atoi(argv[7]);
	else
		return;	
...
void print_run_data(int boids, int loops, int k, double maxv,
					double acc, int width, int height, double sf1, 
					double sf2, double min, double sf3, double sf4)
{
	printf("#header\n");
	printf("boids:%d;loops:%d;k:%d;maxv:%.3lf;acc:%.3lf;width:%d;height:%d;sf1:%.3lf;sf2:%.3lf;min:%.3lf;sf3:%.3lf;sf4:%.3lf\n",
			boids, loops, k, maxv, acc, width, height, sf1, sf2, min, sf3, sf4);
	printf("#endheader\n");
}
          ...
          Node **frontRef, Node **backRef)
{
  Node* fast;
  Node* slow;
  if (source==NULL || source->next==source)
  {
    *frontRef = source;
    *backRef = NULL;
  }
  else
  {
    ...
    *backRef = slow->next;
	...
	source->prev->next = slow->next;
	slow->next->prev = source->prev;
...
struct Node* sortedMerge(Boid **c, Node *a, Node *b)
{
	Boid *boid = (*c);
	Node *head = NULL;
	Node *rem = NULL;
	...
	if (a == NULL)
	{
		b->distance = dist(boid, b->boid);
		return(b);
	}
	else if (b==NULL)
	{
		a->distance = dist(boid, a->boid);
		return(a);
	}
	...
	if (a->distance < b->distance)
	{
		...
		head->next = sortedMerge(&boid, rem, b);
	...
	else
	{
		...
		head->next = sortedMerge(&boid, a, rem);
		head->next->prev->next = head;
		head->prev = head->next->prev;
		head->next->prev = head;
...
void m_sort(Boid **c, Node **headRef)
{
	Boid *boid = (*c);
	Node *head = *headRef;
	Node *a;
...
END PROCEDURE */
Vector * rule1(Boid *b, Node **k, int n, double sf1)
{
	Node *knn = (*k);
	Node *cur = knn;
...
PROCEDURE rule2(boid b)
	...
	RETURN dv
END PROCEDURE */
Vector * rule2(Boid *b, Node **k, int n, double sf2, double min)
{
	Node *knn = (*k);
	Node *cur = knn;
	Vector *bpos = (Vector *)malloc(sizeof(Vector));
...
END PROCEDURE */
Vector * rule3(Node **k, int n, double sf3)
{
	Node *knn = (*k);
	Node *cur = knn;
	Vector * dv = (Vector *)malloc(sizeof(Vector));
...
PROCEDURE rule4(Boid b)
	...
	RETURN v
END PROCEDURE */
Vector * rule4(Boid *b, int xmax, int ymax, double sf4)
{
	Vector * dv = (Vector *)malloc(sizeof(Vector));
	dv->x = 0;
	dv->y = 0;
...
END PROCEDURE */ 
void update_boid(Boid **b, Node **k, int n, int w, int h, double acc, double maxv, double sfs[5])
{
	Boid *boid = (*b);
	Node *knn = (*k);
	Vector *v1, *v2, *v3, *v4;
...
#ifdef RULE2	
	// Apply rule 2
	v2 = rule2(boid, &knn, n, sfs[1], sfs[2]);
	addv(&dv, v2);
	free(v2);
#endif	
...
void Server(int size, int argc, char ***argv)
{
	...
	double sf1 = SF1;		  // Scale factor for rule 1
	double sf2 = SF2;		  // Scale factor for rule 2
	double min = MIN;		  // Minimum distance for rule 2
	double sf3 = SF3;		  // Scale factor for rule 3
	double sf4 = SF4;  		  // Scale factor for rule 4
	...
	for ( loop = 0; loop < loops; loop++ )
	{
		...
		while( !quit ) 
		{
			...
			print_node_list(knn);
				...
				update_boid(&boid, &knn, k, width, height, acc, maxv, sfs);
